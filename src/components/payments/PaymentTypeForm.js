...
const PaymentTypeForm = (props) => {
  ...
  const handleSubmit = (evt) => {
    ...
    let inputDate = new Date(paymentType.expirationDate);
    if (inputDate < newDate) {
      window.confirm("The expiration date is invalid");
    } else {
      PaymentTypeManager.createPaymentType(newPaymentType).then(() =>
  ...
  return (
    <div className="content">
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="button"
          onClick={() => props.history.push("/paymenttypes")}
        >
          Cancel
        </button>
      </form>
